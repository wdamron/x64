...
// Set the executable code for dstAddr.
//
...
func MakeFunc(dstAddr interface{}, executable []byte) error {
	...
	if !v.IsValid() || v.Kind() != reflect.Ptr || v.IsNil() || !v.Elem().CanSet() || v.Elem().Kind() != reflect.Func {
		return fmt.Errorf("Destination for MakeFunc must be a pointer to a function-value")
	}
