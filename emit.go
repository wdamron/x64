...
func checkRex(plen int, argp [8]byte, argc int, args *[4]Arg, rexW bool) (bool, error) {
	requiresRex := rexW
	requiresNoRex := false
	...
	for pi, ai := 0, 0; pi+1 < plen && ai < argc; pi, ai = pi+2, ai+1 {
		...
		if t >= 'a' && t <= 'z' {
			...
			case Mem:
				if v.Base != 0 {
					requiresRex = requiresRex || v.Base.IsExtended()
				}
				if v.Index != 0 {
					requiresRex = requiresRex || v.Index.IsExtended()
				}
			}
		}
	}
...
func emitRex(buf *buffer, r, rm Arg, rexW bool) {
...
func emitVexXop(buf *buffer, e enc, ext extractedArgs, mapSel, pref uint8, rexW, vexL bool) {
